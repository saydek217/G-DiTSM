...
if __name__ == '__main__':
    ...
    for (filename_input, filename_output) in zip(files_input, files_output):
        ...
        for i in range(len(folders)):
            ...
            dir_files = os.listdir(os.path.join('./datas/jester/20bn-jester-v1/', curFolder))
            output.append('%s %d %d' % ('./datas/jester/20bn-jester-v1/' + curFolder, len(dir_files), curIDX))
    ...
    for i in range(len(folders)):
        curFolder = folders[i]
        dir_files = os.listdir(os.path.join('./datas/jester/20bn-jester-v1/', curFolder))
        output.append('%s %d' % ('./datas/jester/20bn-jester-v1/' + curFolder, len(dir_files)))
        print('%d/%d' % (i, len(folders)))
