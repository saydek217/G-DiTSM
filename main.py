...
def main():
    ...
    # train
    if args.mode == 'train':
        log_dir = os.path.join(args.root_log, args.store_name, time_stamp)
        os.makedirs(log_dir, exist_ok=True)  # 👈 This ensures the full folder path exists
        log_training = open(os.path.join(log_dir, 'log.csv'), 'w')
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if (epoch + 1) % args.eval_freq == 0:
                prec1, _ = validate(val_loader, model, criterion, epoch, log_training, tf_writer)
...
def validate(val_loader, model, criterion, epoch, log, tf_writer):
    ...
    output = ('Validate: Prec@1 {top1.avg:.2f} Validate: Prec@5 {top5.avg:.2f}  Loss {loss.avg:.3f}'.format(top1=top1, top5=top5, loss=losses))
    print(output)
    log.write(output + '\n')
    log.flush()
...
def test(test_loader, model, checkpoint, time_stamp):
    model.load_state_dict(torch.load(checkpoint,weights_only=False)['state_dict'])
    model.eval()
    labels = []
        # make sure our output directory exists
    result_dir = os.path.join(args.root_log, args.store_name, time_stamp)
